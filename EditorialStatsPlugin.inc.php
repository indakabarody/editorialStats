...
class EditorialStatsPlugin extends GenericPlugin {
	...
	public function register($category, $path, $mainContextId = NULL) {
		...
		if ($success && $this->getEnabled()) {
			HookRegistry::register('Templates::Index::journal', array($this, 'displayStats'));
		}
		return $success;
	}
	 ...
	 */
	public function displayStats($hookName, $params) {
		$smarty =& $params[1];
		$output =& $params[2];
		...
		// 1. Total submissions
		$totalSubmissions = Capsule::table('submissions')
			->where('context_id', $contextId)
			->count();

		// 2. Published
		$published = Capsule::table('submissions')
			->where('context_id', $contextId)
			->where('status', $STATUS_PUBLISHED)
			->count();

		// 3. In Progress (Queued)
		$inProgress = Capsule::table('submissions')
			->where('context_id', $contextId)
			->where('status', $STATUS_QUEUED)
			->count();

		// 4. Declined
		$declined = Capsule::table('submissions')
			->where('context_id', $contextId)
			->where('status', $STATUS_DECLINED)
			->count();
		...
		$publishedSubmissions = Capsule::table('submissions as s')
			->join('publications as p', 's.submission_id', '=', 'p.submission_id')
			->where('s.context_id', $contextId)
			->where('s.status', $STATUS_PUBLISHED)
			->whereNotNull('p.date_published')
			->select('s.date_submitted', 'p.date_published')
			->get();
		...
		// 7. Reviews completed
		$reviewsCompleted = Capsule::table('review_assignments as r')
			->join('submissions as s', 'r.submission_id', '=', 's.submission_id')
			->where('s.context_id', $contextId)
		...
		// 8. Active reviewers
		$activeReviewers = Capsule::table('review_assignments as r')
			->join('submissions as s', 'r.submission_id', '=', 's.submission_id')
			->where('s.context_id', $contextId)
			->whereNull('r.date_completed')
			->where('r.declined', 0)
			->distinct('r.reviewer_id')
			->count('r.reviewer_id');

		// 9. Submissions per year
		$allSubmissions = Capsule::table('submissions')
			->where('context_id', $contextId)
		...
		$publishedArticles = Capsule::table('submissions as s')
			...
			->leftJoin('section_settings as ss', function($join) use ($currentLocale, $primaryLocale) {
				$join->on('ss.section_id', '=', 'sec.section_id')
